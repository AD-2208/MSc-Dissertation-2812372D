function cntrlLogic(storePath, world, dim)
...
% Plot the world
if dim == 2
    N = 100; % Number of points used to plot circles
    theta = 0:2*pi/N:2*pi; % Angles for a circle
    for i = 1:world.NumObstacles
        X = world.radius(i) * cos(theta) + world.cx(i); % Obstacle X coordinates
        Y = world.radius(i) * sin(theta) + world.cy(i); % Obstacle Y coordinates
        fill(Y, X, 'blue'); % Draw filled obstacles
    end
end

% Plot each agent's path
for agt = 1:length(storePath)
    plot(storePath(agt).path(:,2), storePath(agt).path(:,1), 'g-', 'LineWidth', 0.5);

    % Plot each waypoint as a blk point
    plot(storePath(agt).path(:,2), storePath(agt).path(:,1), 'black.', 'MarkerSize', 10);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
x(7,1) = 1; x(7,2) = 1; x(7,3) = 1; x(7,4) = 1; x(7,5) = 1;
x(8,1) = 1; x(8,2) = 3; x(8,3) = 5; x(8,4) = 7; x(8,5) = 9;
...
for t = 0:dt:total_time
    ...
    for i = 1:numPaths
        ...
        plot(currentY(i), currentX(i), 'red.','MarkerSize', 0.3);
