...
dim = 2; %2D
stepSize = 0.5;
searchRadius = 1; % node search radius
samples = 6000; %node generation
show_output = 1;
start = [4,1; 3,1; 2,1; 1,1; 0,1];
goal = [18,1; 19,1; 20,1; 21,1; 22,1];
totruntime = 0;

[world,rf,ss1,ss2,ss3,ss4] = createMap();
...
for i = 1:agtCount
    tic;
    if i == 1
        [path,pathDist] = RRTstarFn(dim,stepSize,searchRadius,show_output,samples,start(i,:),goal(i,:),world,rf,ss1,ss2,ss3,ss4,i);
        ...
        disp('check')
        safePath = 0;attempt_count = 1;crashTrue = 0;
        while safePath == 0
            [path,pathDist] = RRTstarPP(dim,stepSize,searchRadius,show_output,samples,start(i,:),goal(i,:),world,rf,ss1,ss2,ss3,ss4,i);
            [xPos, yPos, time] = cntrl(path, start(i,:));
            xCoords{i} = xPos;
            yCoords{i} = yPos;
            tCoords{i} = time;
